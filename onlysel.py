...
from util import to_df,KL_Loss,mean_Loss,fd_calculated,sel_loss,sel_net,boolean_conv
...
def V_Train(t, path, sampleloader, G, D, name,
epochs, lr, dataloader, z_dim, dataset, 
col_type, sample_times, itertimes = 100, 
steps_per_epoch = None, GPU=False, KL=True, mean = True, fd = False, sel=True, verbose = False):
        ...
        while it < steps_per_epoch:
            ...
            for x_real in dataloader:
                ...
                if fd:
                    ...
                    G_fd = fd_calculated(df_fake,fd,y_fake,fd_model,fd_graph,fd_session)
                else:
                    G_fd = 0
                ...
                if it >= steps_per_epoch:
                    ...
                    for time in range(sample_times):
                        ...
                        sample_data.to_csv(path+'{}_{}_{}_{}.csv'.format(name,t,epoch,time), index = None)
                    if GPU:
                        G.cuda()
                        G.GPU = True
                    G.train()
...
def generation(dataname,mean, fd, sel, name):
    ...
    V_Train(search, path, sample_it, gen, dis, name,
        1000, param["lr"], train_it, param["z_dim"], trn, col_type, 
        sample_times, itertimes = 100, steps_per_epoch = config["steps_per_epoch"], GPU=GPU, 
        KL=KL, mean = boolean_conv(mean), fd = boolean_conv(fd), sel = boolean_conv(sel))


generation(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5])
