...
try:
    ...
    data = np.load(path + filename + "_transformed.npy")
    query = np.load(path + filename + "_transformed.npy")
...
test_data_predictions_labels_file = os.path.join('./{}/'.format(filename), 'test_predictions.npy')
valid_data_predictions_labels_file = os.path.join('./{}/'.format(filename), 'valid_predictions_labels_one_epoch_')

regression_name = filename
regression_model_dir = 'pretrained/'+ filename +"/sel/{}".format(sys.argv[1])
