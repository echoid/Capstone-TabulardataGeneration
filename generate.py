...
from util import to_df,KL_Loss,mean_Loss,sel_loss,sel_net,boolean_conv
...
config["train"] ="dataset/origin/{}.csv".format(sys.argv[1])
config["sample"] = "dataset/origin/{}.csv".format(sys.argv[1])
    ...
    for epoch in range(epochs):
        ...
        while it < steps_per_epoch:
            ...
            for x_real in dataloader:
                ...
                G_Loss = G_origin + G_mean + G_KL + G_sel

                log = open(path+"train_log"+".txt","a+")
                log.write("{},{},{},{},{},{}\n".format(epoch,it,G_origin,G_mean,G_KL,G_sel))
                log.close()
...
def generation(dataname,mean, sel, name):
    ...
    V_Train(search, path, sample_it, gen, dis, name,
        ...
        KL=KL, mean = boolean_conv(mean),sel = boolean_conv(sel))
